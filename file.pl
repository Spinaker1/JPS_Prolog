...
goal_achieved(on(X,Y/on(Y,Z)), State) :- \+ compound(Z), goal_achieved(on(Y,Z), State), member1(on(X,Y), State).
goal_achieved(diff(Z,X/on(X,Y)), State) :- \+ compound(Z), goal_achieved(on(X,Y), State), Z \= X.
...
inst_action(Action, Conditions, State1, InstAction) :- change_structures_to_simple_var(Action,InstAction),goals_achieved(Conditions,State1).
